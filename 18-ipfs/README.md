...
* **verify signature**: it proves the user owns the RSA key pairs generated in IPFS
	* user signs his Ethereum wallet address using private key;
	* pass the wallet address, public key along with signature to Ocean;
	* Ocean verify the signature is generated using correct keypairs and the decrypted message matches with the received wallet address.
* **verify IPFS node id**: it proves the registered user is the authentic IPFS node 
	* the key is `IPFS node id is generated by hashing RSA public key`;
	* Ocean had verified user owns the RSA keypairs in the last step;
	* furthermore, Ocean can verify the IPFS node id by hashing the RSA public key.

The validation logic flow is:
...
* Users come to Ocean marketplace to explore valuable dataset;
* When requesting a specific dataset, the IPFS url is returned to users;
* Users download the dataset file from IPFS. 
	* Note that users may download different pieces of data file from many different IPFS nodes.
	* IPFS will connect users with neighboring IPFS nodes who had the dataset.
	* IPFS guarantees the data integrity and verify the data availability.

<img src="img/accessData.jpg" width=600/>  

### 5.3 Distribute Rewards

The reward distribution flow can be significantly simplied because IPFS guarantees the data availability, therefore, Ocean does not need verifier network to challenge the data provider any more.

The workflow can be shown as below:

<img src="img/distribution.jpg" width=800/>  
...
* **Token Distribution**:
	...
	* The actual reward amount is calculated according to formula considering:
		* dataset size, staking amount, number of downloads, ...
 ...
 * who can be trusted inside Ocean network to pull out the provider list from IPFS? 
 * When pulling out the provider list, the IPFS node tends to find its cloest neighboring IPFS nodes. As such, is there any chance of biased result? Shall we gather the list from multiple IPFS nodes from various locations?
